...
A login experience inspired by [pika.me/login](https://pika.me/login), built with Next.js 14, TypeScript, Tailwind CSS, and Framer Motion. Implements a full auth flow backed by a mock API layer with typed error handling and session persistence.
...
> **Important:** Always use `npm run dev` for local development. Do not run `npm run build` before `npm run dev` in the same session — the production `.next` artifacts are incompatible with the dev server's chunk numbering and will cause 500 errors. If you ever see `Cannot find module './XXX.js'`, run `rm -rf .next` and restart `npm run dev`.
...
|---|---|---|
| Framework | Next.js 14 (App Router) | Named in JD; middleware for route protection |
| Language | TypeScript | Discriminated unions for error modeling; no `any` |
| Styling | Tailwind CSS + CSS custom properties | Design token layer for consistent theming |
| Animations | Framer Motion | Smooth step transitions, entry animations, tap feedback |
| State | React Context + `useReducer` | Right scope for auth — no over-engineering |
| Session | `localStorage` + cookie | localStorage for client hydration; cookie enables server-side middleware guard |
...
│   │   └── DevHints.tsx      # Dev-only floating panel for error simulation (hidden in production)
│   └── ui/
│       ├── Input.tsx         # Labeled input with inline error + aria
...
├── context/
│   └── AuthContext.tsx       # Auth state, localStorage persistence, session hydration
...
A **DEV panel** is available in development — look for the pulsing `DEV` button fixed to the bottom-right corner of the login page. It shows all trigger values with one-click copy. It is not rendered in production builds (`NODE_ENV !== "development"`).
...
The auth state surface is narrow: one user object, one token, login, and logout. React Context with `useReducer` is the right tool here. Zustand would add a dependency and abstraction layer where none is needed — knowing when not to reach for a library is itself a design decision.
...
When all 6 OTP digits are filled, verification fires automatically. This removes the need for an extra button press, matching the pattern used by most production OTP flows (WhatsApp, Apple, Stripe).
